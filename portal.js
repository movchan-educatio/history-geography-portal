...
function renderResources(){const box=$('resourcesList');const arr=Object.entries(resources).sort((a,b)=>(b[1].createdAt||0)-(a[1].createdAt||0));box.innerHTML=arr.length?arr.map(([id,x])=>`<article class="item"><div class="item-head"><div><span class="pill">${esc(x.type||'Матеріал')}</span><h3>${esc(x.title)}</h3></div><span class="pill">${esc(x.ownerName||'Портал')}</span></div><p>${esc(x.description||'')}</p><p>🔗 <a href="${esc(x.url)}" target="_blank" rel="noopener">Відкрити матеріал</a></p></article>`).join(''):'<div class="empty">Матеріалів ще немає.</div>'}
function renderGrades(){const box=$('gradesList');if(profile?.role==='teacher'){const arr=Object.entries(grades);box.innerHTML=arr.length?arr.map(([uid,x])=>`<article class="item"><div class="item-head"><div><h3>${esc(x.studentName||uid)}</h3><p>${esc(x.title||'Робота')}</p></div><span class="grade">${esc(x.grade||'—')}</span></div><p>${esc(x.feedback||'')}</p></article>`).join(''):'<div class="empty">Оцінок ще немає.</div>'}else{const arr=Object.values(grades).filter(x=>x.studentUid===user?.uid);box.innerHTML=arr.length?arr.map(x=>`<article class="item"><div class="item-head"><div><h3>${esc(x.title||'Робота')}</h3><p>${esc(x.feedback||'')}</p></div><span class="grade">${esc(x.grade||'—')}</span></div></article>`).join(''):'<div class="empty">Ваші оцінки з’являться тут після перевірки робіт.</div>'}}
function renderSubmissions(){const box=$('submissionsList');if(!profile)return;const arr=Object.entries(submissions).filter(([id,x])=>profile.role==='teacher'||x.studentUid===user.uid);box.innerHTML=arr.length?arr.map(([id,x])=>`<article class="item"><div class="item-head"><div><span class="pill">${profile.role==='teacher'?esc(x.studentName||'Учень'):'Моя робота'}</span><h3>${esc(x.assignmentTitle||'Завдання')}</h3></div><span class="grade">${x.grade?esc(x.grade):'—'}</span></div><p>${esc(x.text||'Файл/текст подано')}</p>${x.fileUrl?`<p>📎 <a href="${esc(x.fileUrl)}" target="_blank">Відкрити файл</a></p>`:''}${profile.role==='teacher'?`<div class="form-actions"><button class="btn-secondary" onclick="gradeSubmission('${id}')">Оцінити</button></div>`:''}</article>`).join(''):'<div class="empty">Поданих робіт поки немає.</div>'}

async function saveLesson(e){e.preventDefault();if(profile?.role!=='teacher')return toast('Ця дія доступна лише вчителю.');const f=e.target;const id=push(ref(db,'lessons')).key;await set(ref(db,'lessons/'+id),{title:f.title.value,subject:f.subject.value,grade:f.grade.value,topic:f.topic.value,homework:f.homework.value,videoUrl:f.videoUrl.value,presentationUrl:f.presentationUrl.value,createdBy:user.uid,createdAt:Date.now()});f.reset();toast('Урок опубліковано');}
async function saveAssignment(e){e.preventDefault();if(profile?.role!=='teacher')return toast('Ця дія доступна лише вчителю.');const f=e.target;const id=push(ref(db,'assignments')).key;await set(ref(db,'assignments/'+id),{title:f.title.value,description:f.description.value,dueAt:f.dueAt.value,resourceUrl:f.resourceUrl.value,createdBy:user.uid,createdAt:Date.now()});f.reset();toast('Домашнє завдання створено');}
async function addResource(e){e.preventDefault();if(!user)return;const f=e.target;let url=f.url.value;const file=f.file.files[0];if(file){toast('Завантаження файлу…');const sref=storageRef(storage,`portal-resources/${user.uid}/${Date.now()}-${file.name}`);await uploadBytes(sref,file);url=await getDownloadURL(sref)}if(!url)return toast('Додайте посилання або файл.');const id=push(ref(db,'resources')).key;await set(ref(db,'resources/'+id),{title:f.title.value,type:f.type.value,description:f.description.value,url,ownerUid:user.uid,ownerName:user.displayName||'Користувач',createdAt:Date.now()});f.reset();toast('Матеріал додано');}
window.openSubmission=id=>{const a=assignments[id];if(!a)return;if(profile?.role==='teacher'){document.querySelector('[data-tab="submissions"]').click();return}$('submissionAssignmentId').value=id;$('submissionTitle').textContent=a.title;document.querySelector('[data-tab="submit"]').click()}
async function submitWork(e){e.preventDefault();if(!user)return toast('Увійдіть через Google.');const f=e.target,id=f.assignmentId.value,a=assignments[id];let fileUrl='';const file=f.file.files[0];if(file){toast('Завантаження роботи…');const sref=storageRef(storage,`submissions/${user.uid}/${Date.now()}-${file.name}`);await uploadBytes(sref,file);fileUrl=await getDownloadURL(sref)}const sid=push(ref(db,'submissions')).key;await set(ref(db,'submissions/'+sid),{assignmentId:id,assignmentTitle:a.title,studentUid:user.uid,studentName:user.displayName||'Учень',text:f.text.value,fileUrl,submittedAt:Date.now(),grade:'',feedback:''});f.reset();toast('Роботу надіслано вчителю');document.querySelector('[data-tab="submissions"]').click()}
window.gradeSubmission=async id=>{const x=submissions[id];const grade=prompt('Оцінка (наприклад 11 або 11/12):',x.grade||'');if(grade===null)return;const feedback=prompt('Короткий коментар учня:',x.feedback||'')||'';await update(ref(db,'submissions/'+id),{grade,feedback,gradedAt:Date.now(),gradedBy:user.uid});const gid=id;await set(ref(db,'grades/'+gid),{studentUid:x.studentUid,studentName:x.studentName,title:x.assignmentTitle,grade,feedback,gradedAt:Date.now()});toast('Оцінку збережено');}

window.addEventListener('DOMContentLoaded',()=>{
 document.querySelectorAll('.tab-btn').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('.tab-btn').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.tab-panel').forEach(x=>x.classList.remove('active'));b.classList.add('active');$(b.dataset.tab).classList.add('active')}));
